...
string WriteCommandExecutor::distortString (string s)
{
	int i = 0;
	while (i != s.size()){
		if (s.at(i) == '<'){
...
Result* WriteCommandExecutor::executeCommand (TaskList* mainTaskList, Command* command)
{
	string filename;
...
Result* WriteCommandExecutor::executeCommand(TaskList* mainTaskList, Result* result, Command *command){
	string filename;
	if (command->method == WRITE){
