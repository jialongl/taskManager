...
This part of the manual is mainly meant for developers. Users who are interested in finding
out the actual structure behind are also welcome to read this part.

\subsection{Overview of Development}
We strictly followed a top-down approach designing the software. Regarding the software
development life cycle (SDLC), our team is following the iterative approach on the whole. In each
iteration, we are following the waterfall approach so that every
version of our software is a complete and working product that satisfies our
target users.
% At the beginning of each iteration of the SDLC, we specify the requirements and functionalities of this iteration.
...
user is able to type in “add "write developer's guide" -t 1d” to add in a new task "write
developer's guide" which has a deadline of 1 day.

We did some product surveys to find out what are the common features that existing products
...
The focus group of this project are mainly Windows users. By conducting interviews with typical
Windows users, we have also decided to make a TUI (text user interface) in order to create a
more user friendly environment. Details are given in section 2.4.
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.4]{a-condensed-OO-domain-model}
...
Figure 6. Code fragments from IO Module
\end{enumerate}
...
\begin{enumerate}
\item Task
The task class is the main class that stores information for the user. The diagram describing
...
\item Task List
Task List class is a class that links multiple instances of Task objects to form a list which is easy
to maintain and manipulate. The following diagram illustrates the structure of Task List class.
...
\subsection{Use cases in sequence diagrams}
Assume we are adding a task ``Write developer’s guide'' with deadline ``1d''.
Top level:

A more detailed use case inside Logic part is shown as follows:

The actual use case inside the UI part is given as follows:
