...
\item Remove task(s): \cmd{\begin{tabbing}
    > \ ls\\
    1 f \hspace{0.16in} \= Sample task 1\\
    2   \>Sample task 2\\
    > \ rm 1\\
...
\begin{tabbing}
  > \ ls\\
    ...
    > \ read thisweek.xml\footnotemark \\
...
\footnotetext{ Currently \cmdinline{export} does not support environmental variables in path. i.e. \cmdinline{export \~{}/abc.xml} will not export the file to user's home directory /home/username/.}
 
\cmdinline{export} can also be used to generate an HTML file which is more visually pleasant in your favourate browser.
\cmd{> \ export -html sampletasks.html\\
  > \ exit}
...
\begin{enumerate}
\item [ \cmdinline{-t} ] add a task with a deadline: \cmd{> \ add "some task" -t 3d2h\\
  > \ add "some task" -t b2d\\
...
\emph{Options are not compulsory. Different options can be used together.} For example: \cmd{> \ add "CS2103 final exams" -p 10 -g "finals" -t b3w2d} would add a task ``CS2103 final exam'' which has a priority of 10, belongs to group ``finals'' and is held on the Tuesday 3 weeks from now.
...
\item [ \cmdinline{-k} ] filters tasks with a keyword\footnotemark \ where ? means any single character, * means a string of any length (including 0 length).
\footnotetext{ keyword is case insensitive.}

Take command \cmdinline{ls -k *Sam?le*task} as an example, ``This is a sample with a important task'' will match *Sam?le*task as the first * matches ``This is a '', ? matches `p' and the second * matches `` with a important ''.
...
\begin{quote}
Different options can be used together.
When more then one restrictive options are there, \emph{conjuction} of these restrictions are used. e.g. \cmd{ls -g SampleTask -f y} will show tasks that are finished AND from ``SampleTask'' group. 
\end{quote}
...
%\includegraphics[scale=0.62]{tui-launched}
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.62, viewport=50 0 600 450]{tui-launched}
...
TaskManager supports custom command mapping/aliasing. General format of \cmdinline{map} is: \cmdinline{map "new command" "original command"}
...
Task manager commands can be saved in a single script file and be executed using run command. \cmd{
\begin{tabbing}
  \$ cat tmscript\\
  ls\\
  map "ls" "ls -f no"\\
...
Example: \cmd{> \ what do I do today?} will list all the tasks due today.
...
On Windows, taskManager can be built with Visual Studio 2008\registered\footnotemark \ in the following steps:
