...
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.73, viewport=120 0 600 380]{exporthtml}
...
\begin{tabular}{c | c}
  ...
  \cmdinline{number g} & select the \cmdinline{number}th task displayed in the list\\
  \cmdinline{number G} & select the task whose serial number is \cmdinline{number}\\
...
For all inputs that cannot be recognized by taskManager as a command, it will be treated as natural language sentence. 
